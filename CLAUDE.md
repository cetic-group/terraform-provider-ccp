...
**Latest** : `v6.3.1` (2026-09-04) — fix : **un réseau isolé ne refuse plus un script de démarrage** (#67). `snatvalidator` rendait une ERREUR DURE de plan dès qu'une ressource de calcul portait un `user_data` non vide dans un sous-réseau à `snat = false` ; il rend désormais un AVERTISSEMENT. Deux raisons : le contrôle ne regarde jamais le CONTENU du script (écrire un fichier ou démarrer un logiciel déjà présent dans le gabarit n'a besoin d'aucune sortie), et surtout il **échouait à l'envers** — il sort en silence sur toute erreur de sonde, donc il bloquait qui il pouvait atteindre et se contournait en coupant l'API le temps d'un plan. Comportement documenté sur `ccp_vnet.snat`, où il ne l'avait JAMAIS été. PR #69.

**Précédent** : `v6.3.0` (2026-09-04) — feat : **DNS privé** (`ccp_dns_zone`, `ccp_dns_record` + data sources `ccp_dns_zone`/`ccp_dns_records`), **messagerie hébergée** (`ccp_email_domain`, `ccp_email_account`, `ccp_email_alias` + 4 data sources) et **réseaux isolés** documentés sur `ccp_vnet.snat` / `ccp_k8s_cluster`. Trois pièges figés dans les schémas : le `tier` DNS appartient au RÉSEAU (toutes les zones d'un `vpc_id` le partagent, 409 sinon) ; `records` est un `Set` qui REMPLACE (l'API rend son propre ordre) ; un domaine public — DNS comme messagerie — naît EN ATTENTE de preuve de possession, d'où `wait_for_verification` et un parcours en DEUX `apply`. Tous les champs que la mise à jour amont ne sait pas EFFACER sont `Optional+Computed` (`quota_gb`, `comment`, `displayed_name`) : simplement `Optional`, les retirer du HCL enregistrait `null` sur une valeur toujours en place et toujours facturée. Exemples et extraits de doc validés contre le schéma COMPILÉ (les dix exemples du dépôt passent `terraform validate` — trois ne compilaient plus). Contraintes portées `~> 5.0` → `~> 6.3` partout. PR #66, issues #65/#63/#64 ; suivi #67 (`snatvalidator`).

⚠️ **Ce bloc a sauté les versions `v5.3.0` → `v6.2.2`** : elles ont été publiées sans mise à jour de ces notes. Se fier au Registry et au `CHANGELOG.md` pour cette plage, pas à cette section.

**Précédent** : `v5.2.0` (2026-06-18) — feat : **version Kubernetes par node pool** — `ccp_k8s_node_pool.k8s_version` (**Optional+Computed**, NON ForceNew → mutable = rolling upgrade ; omis = hérite du control-plane ; readback null quand « hérite ») + `ccp_k8s_cluster.initial_pool.k8s_version`. Contrainte serveur : worker `<=` control-plane, sinon 422. Additif non-breaking.
