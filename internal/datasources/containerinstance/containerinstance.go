// Package containerinstance implements the ccp_container_instance data source.
...
func New() datasource.DataSource { return &containerDataSource{} }

type containerDataSource struct{ client *client.Client }

type containerDSModel struct {
...
func (d *containerDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Look up an LXC container instance by `id`, or by `(name, region)`.",
...
func (d *containerDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if hasID {
		got, err := d.client.GetContainer(ctx, cfg.ID.ValueString())
		if err != nil {
		...
		list, err := d.client.ListContainers(ctx, cfg.Region.ValueString())
		if err != nil {
		...
		switch len(matches) {
		case 0:
			resp.Diagnostics.AddError("Container not found", fmt.Sprintf("No container named %q in region %q.", wantName, wantRegion))
			return
		case 1:
			m := list[matches[0]]
			found = &m
		default:
			resp.Diagnostics.AddError("Multiple containers matched", fmt.Sprintf("Found %d containers named %q in region %q.", len(matches), wantName, wantRegion))
			return
		}
	}

	state := containerDSModel{
