// Package dbmysqlinstance implements the ccp_db_mysql_instance data source.
...
var (
	_ datasource.DataSource              = (*mysqlDS)(nil)
	_ datasource.DataSourceWithConfigure = (*mysqlDS)(nil)
)

func New() datasource.DataSource { return &mysqlDS{} }

type mysqlDS struct{ client *client.Client }

type mysqlDSModel struct {
...
func (d *mysqlDS) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Look up a MariaDB/MySQL database instance by `id` or `name`. Credentials are NOT surfaced — use `ccp_db_mysql_credentials`.",
...
func (d *mysqlDS) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *mysqlDS) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg mysqlDSModel
	...
	var found *client.DbMysqlInstance
	if hasID {
		got, err := d.client.GetDbMysql(ctx, cfg.ID.ValueString())
		if err != nil {
		...
		list, err := d.client.ListDbMysql(ctx)
		if err != nil {
			resp.Diagnostics.AddError("Failed to list MySQL instances", err.Error())
			return
		}
		...
		switch len(matches) {
		case 0:
			resp.Diagnostics.AddError("MySQL instance not found", fmt.Sprintf("No instance named %q.", want))
			return
		case 1:
			m := list[matches[0]]
			found = &m
		default:
			resp.Diagnostics.AddError("Multiple MySQL instances matched", fmt.Sprintf("Found %d instances named %q.", len(matches), want))
			return
		}
	}
