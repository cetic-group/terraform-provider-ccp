// Package dbferretdbinstance implements the ccp_db_ferretdb_instance data source.
...
var (
	_ datasource.DataSource              = (*ferretDS)(nil)
	_ datasource.DataSourceWithConfigure = (*ferretDS)(nil)
)

func New() datasource.DataSource { return &ferretDS{} }

type ferretDS struct{ client *client.Client }

type ferretDSModel struct {
...
func (d *ferretDS) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "ccp_db_ferretdb_instance"
}

func (d *ferretDS) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Look up a FerretDB (MongoDB-compatible) instance by `id` or `name`. Credentials are NOT surfaced.",
		Attributes: map[string]schema.Attribute{
			...
			"admin_database":    schema.StringAttribute{Computed: true},
...
func (d *ferretDS) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ferretDS) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg ferretDSModel
	...
	var found *client.DbFerretdbInstance
	if hasID {
		got, err := d.client.GetDbFerretdb(ctx, cfg.ID.ValueString())
		if err != nil {
		...
		list, err := d.client.ListDbFerretdb(ctx)
		if err != nil {
			resp.Diagnostics.AddError("Failed to list FerretDB instances", err.Error())
			return
		}
		...
		switch len(matches) {
		case 0:
			resp.Diagnostics.AddError("FerretDB instance not found", fmt.Sprintf("No instance named %q.", want))
			return
		case 1:
			m := list[matches[0]]
			found = &m
		default:
			resp.Diagnostics.AddError("Multiple FerretDB instances matched", fmt.Sprintf("Found %d instances named %q.", len(matches), want))
			return
		}
	}
	...
	setStrPtr(&state.AdminUsername, found.AdminUsername)
	setStrPtr(&state.AdminDatabase, found.AdminDatabase)
