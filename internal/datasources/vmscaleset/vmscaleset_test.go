...
func TestDetail_PorteLesMembres(t *testing.T) {
	srv := testutil.NewTestServer(t, testutil.Routes{
		{Method: "GET", Path: "/v1/vm-scale-sets/ss-1", Status: http.StatusOK, Body: map[string]any{
			"id": "ss-1", "name": "workers", "region": "RNN", "plan": "small", "template": "ubuntu-24.04",
			"min_instances": 1, "max_instances": 5, "desired_instances": 2, "auto_repair": false,
			"status": "active", "tags": []string{},
			"created_at": "2026-05-25T10:00:00Z", "updated_at": "2026-05-25T10:05:00Z",
			"vms": []map[string]any{
				{"id": "c-1", "name": "workers-1", "status": "running", "ip_address": "10.0.1.11"},
				...
				{"id": "c-2", "name": "workers-2", "status": "creating"},
			},
		}},
	})
	defer srv.Close()
	...
	if len(got.VMs) != 2 {
		t.Fatalf("le détail ne porte pas ses membres : %d", len(got.VMs))
	}
	if got.VMs[0].IPAddress == nil || *got.VMs[0].IPAddress != "10.0.1.11" {
		t.Fatalf("IP du premier membre perdue : %+v", got.VMs[0])
	}
	if got.VMs[1].IPAddress != nil {
		t.Fatalf("un membre sans IP doit rester null, pas %q", *got.VMs[1].IPAddress)
	}
}
...
func TestListe_NePortePasLesMembres(t *testing.T) {
	srv := testutil.NewTestServer(t, testutil.Routes{
		{Method: "GET", Path: "/v1/vm-scale-sets", Status: http.StatusOK, Body: []map[string]any{{
			"id": "ss-1", "name": "workers", "region": "RNN", "plan": "small", "template": "ubuntu-24.04",
			"min_instances": 1, "max_instances": 5, "desired_instances": 2, "auto_repair": false,
			"status": "active", "tags": []string{},
			"created_at": "2026-05-25T10:00:00Z", "updated_at": "2026-05-25T10:05:00Z",
	...
	c := client.New(srv.URL, "ccp_test_unit", "0.0.0-test")
	list, err := c.ListVMScaleSets(context.Background(), "RNN")
	if err != nil {
		t.Fatalf("ListVMScaleSets: %v", err)
	}
	if len(list) != 1 {
...
func TestRead_ProjetteLesMembresDansLEtat(t *testing.T) {
	...
	srv := testutil.NewTestServer(t, testutil.Routes{
		{Method: "GET", Path: "/v1/vm-scale-sets/ss-1", Status: http.StatusOK, Body: map[string]any{
			...
			"created_at": "2026-05-25T10:00:00Z", "updated_at": "2026-05-25T10:05:00Z",
			"vms": []map[string]any{
				{"id": "m-1", "name": "workers-1", "status": "running", "ip_address": "10.0.1.11"},
				{"id": "m-2", "name": "workers-2", "status": "creating"},
			},
		}},
	})
	defer srv.Close()

	d := &vmssDS{client: client.New(srv.URL, "ccp_test_unit", "0.0.0-test")}
	...
	var etat vmssDSModel
	if diags := resp.State.Get(ctx, &etat); diags.HasError() {
		t.Fatalf("State.Get: %v", diags.Errors())
	}
	if etat.VMs.IsNull() || len(etat.VMs.Elements()) != 2 {
		t.Fatalf("les membres n'atteignent pas l'état : %#v", etat.VMs)
	}

	var membres []membreModel
	if diags := etat.VMs.ElementsAs(ctx, &membres, false); diags.HasError() {
...
func TestRead_ParNom_RelitLeDetailPourLesMembres(t *testing.T) {
	...
	detail["vms"] = []map[string]any{
		{"id": "m-1", "name": "workers-1", "status": "running", "ip_address": "10.0.1.11"},
	}
	srv := testutil.NewTestServer(t, testutil.Routes{
		{Method: "GET", Path: "/v1/vm-scale-sets", Status: http.StatusOK, Body: []map[string]any{base}},
		{Method: "GET", Path: "/v1/vm-scale-sets/ss-1", Status: http.StatusOK, Body: detail},
	})
	defer srv.Close()
	...
	champs["name"] = tftypes.NewValue(tftypes.String, "workers")
