...
func (d *qemuTemplatesDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Lists active QEMU/VM templates available on CETIC Cloud (admin-managed catalog, excludes internal `ccks-*` Kubernetes images). Useful for resolving a template `key` (e.g. `ubuntu-24.04-cloud`) to use in `ccp_vm_instance.template`.",
