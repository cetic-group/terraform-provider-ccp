// Package dbpginstance implements the ccp_db_pg_instance data source.
...
var (
	_ datasource.DataSource              = (*pgDataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*pgDataSource)(nil)
)

func New() datasource.DataSource { return &pgDataSource{} }

type pgDataSource struct{ client *client.Client }

type pgDSModel struct {
...
func (d *pgDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		MarkdownDescription: "Look up a PostgreSQL database instance by `id` or `name`. Credentials are NOT surfaced — use `ccp_db_pg_credentials`.",
...
func (d *pgDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *pgDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var cfg pgDSModel
	...
	var found *client.DbPgInstance
	if hasID {
		got, err := d.client.GetDbPg(ctx, cfg.ID.ValueString())
		if err != nil {
		...
		list, err := d.client.ListDbPg(ctx)
		if err != nil {
			resp.Diagnostics.AddError("Failed to list PostgreSQL instances", err.Error())
			return
		}
		...
		switch len(matches) {
		case 0:
			resp.Diagnostics.AddError("PostgreSQL instance not found", fmt.Sprintf("No instance named %q.", want))
			return
		case 1:
			m := list[matches[0]]
			found = &m
		default:
			resp.Diagnostics.AddError("Multiple PostgreSQL instances matched", fmt.Sprintf("Found %d instances named %q.", len(matches), want))
			return
		}
	}
