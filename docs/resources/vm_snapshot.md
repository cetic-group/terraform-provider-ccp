...
Creates a point-in-time snapshot of a VM instance. Snapshots capture the full disk state and are stored on Ceph RBD. They can be used to restore the VM to a known-good state via the console or CLI. Up to 2 free snapshots are included per instance.
