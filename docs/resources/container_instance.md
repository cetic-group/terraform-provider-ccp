...
- `docker` - (Optional, Forces new resource) Required to run Docker inside the container (enables nesting; opt-in). Defaults to `false`.
- `is_template_source` - (Optional, Forces new resource) Create a template-preparation instance (hidden from the container listing, meant to be converted into a custom template then deleted). Defaults to `false`.
