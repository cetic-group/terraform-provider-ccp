...
- `docker` - (Optional, Forces new resource) Enable Docker (nesting) on every replica (opt-in). Defaults to `false`. When disabled, replicas are hardened against host-topology leakage.
