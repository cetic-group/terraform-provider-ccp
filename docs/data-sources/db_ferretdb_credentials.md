---
page_title: "ccp_db_ferretdb_credentials Data Source - cetic-cloud-platform"
subcategory: "Databases"
description: |-
  Fetches the admin credentials of a FerretDB v2 (MongoDB-compatible) managed instance.
---

# ccp_db_ferretdb_credentials (Data Source)

Read-only datasource that fetches the admin credentials of a FerretDB v2 (MongoDB-compatible)
...
```hcl
resource "ccp_db_ferretdb_instance" "db" {
  name           = "app-db"
  region         = "RNN"
...
data "ccp_db_ferretdb_credentials" "db" {
  id = ccp_db_ferretdb_instance.db.id
}

output "db_uri" {
  value     = data.ccp_db_ferretdb_credentials.db.uri
  sensitive = true
}
```
...
- `id` - (Required) UUID of the FerretDB v2 (MongoDB-compatible) instance.
