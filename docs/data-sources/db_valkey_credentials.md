...
# ccp_db_valkey_credentials (Data Source)

Read-only datasource that fetches the admin credentials of a Valkey (Redis-compatible)
managed instance from the CETIC Cloud API. Returns password + host:port + uri (no username/database).
...
```hcl
resource "ccp_db_valkey_instance" "db" {
  name           = "app-db"
  region         = "RNN"
...
data "ccp_db_valkey_credentials" "db" {
  id = ccp_db_valkey_instance.db.id
}

output "db_uri" {
  value     = data.ccp_db_valkey_credentials.db.uri
  sensitive = true
}
```
...
- `id` - (Required) UUID of the Valkey (Redis-compatible) instance.
