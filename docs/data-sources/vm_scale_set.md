---
page_title: "ccp_vm_scale_set Data Source - ccp"
subcategory: "Compute"
description: |-
...
- `vms` - The scale set's members as the platform knows them **at read time**. Each entry carries `id`, `name`, `status` and `ip_address` (`null` while a member has no address yet). Use it to place a scale set behind an Application Gateway or a load balancer:

```hcl
data "ccp_vm_scale_set" "prod" {
  id = ccp_vm_scale_set.prod.id
}

resource "ccp_appgw_target_group_member" "prod" {
  for_each        = { for m in data.ccp_vm_scale_set.prod.vms : m.id => m }
  appgw_id        = ccp_application_gateway.prod.id
  target_group_id = ccp_appgw_target_group.prod.id
  vm_instance_id    = each.value.id
  port            = 8080
}
```
